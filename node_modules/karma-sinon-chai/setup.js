...
// Cross-platform node.js postinstall & prepublish for bower dependencies.
...
switch (mode) {
  ...
  case 'prepublish':
    ...
    execute(getBinaryPath('bower'), 'install');
    break;
  ...
  case 'postinstall':
    fsExists(sysPath.join(__dirname, 'bower_components'), function(exists) {
      if (exists) return;
      execute(getBinaryPath('bower'), 'install');
    });
    break;
